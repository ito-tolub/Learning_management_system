    ...
    <>
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
        ...
        <div className='max-w-xl z-10 text-gray-500'>
          <h1 className='text-2xl md:text-3xl lg:text-4xl  leading-tight font-semibold text-gray-800'>{courseData.courseTitle}</h1>
          <div className="text-gray-600 leading-relaxed" dangerouslySetInnerHTML={{ __html: courseData.courseDescription.slice(0, 200) }}
          />
          {/*review and rating*/}
          ...
          <p>Course by <span className='text-blue-600 underline'>Muhammad Tosan</span></p>
                ...
                <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                  <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={() => toggleSection(index)}>
                    <div className='flex items-center gap-2'>
                      <img src={assets.down_arrow_icon} alt="arrow icon" className={`transition-transform duration-300 ${openSection[index] ? 'rotate-180' : ''}`} />
                          ...
                          <div className='flex items-center justify-between w-full text-gray-800 text-xs md:text-default'>
                            ...
                            <div className='flex gap-2'>
                              {lecture.isPreviewFree && <p
                                onClick={() => setPlayerData({ videoId: lecture.lectureUrl.split('/').pop() })} className='text-blue-500 hover:underline cursor-pointer'>Preview</p>}
        ...
        {/*Right columns*/}
        <div className='max-w-course-card z-10 
shadow-lg shadow-black/10 
rounded-xl overflow-hidden bg-white w-full md:w-[400px] lg:w-[420px]'>

          {playerData ? <YouTube videoId={playerData.videoId} opts={{ playerVars: { autoplay: 1 } }} iframeClassName='w-full aspect-video' /> : <img src={courseData.courseThumbnail} alt="" className='w-full aspect-video object-cover' />}
          ...
          <div className='p-5 '>
            ...
            <button
              onClick={!isAlreadyEnrolled ? handleEnroll : undefined}
