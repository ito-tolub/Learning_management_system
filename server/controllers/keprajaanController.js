...
export const syncKeprajaan = async (req, res) => {
  try {
    ...
    const keprajaan = await Keprajaan.findOne({ npp: nppStr });
    ...
    if (!keprajaan) {
      ...
      console.log(
        "Contoh data di DB:",
        semua.map((k) => ({ npp: k.npp, type: typeof k.npp })),
      );
      return res.json({
        success: false,
        message: "NPP tidak ditemukan dalam sistem",
      });
    }
    ...
    res.json({
      success: true,
      user,
      message: "Data keprajaan berhasil disinkronkan",
    });
  } catch (error) {
